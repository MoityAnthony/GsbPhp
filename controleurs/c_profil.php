...
switch($action){
	...
	case 'updateMDP':{
		if(isset($_POST["valider"]) && isset($_POST['mdp']))
		{
			try{
					$mdp=$_POST['mdp'];
					$mdp = password_hash($mdp,PASSWORD_BCRYPT,['cost' => 10]);
					$pdo->UpdateLePassword($mdp,$id);
					header("Location:index.php?uc=profil&action=Information");
				}
				catch(PdoExecptions $e)
				{
	...
	case 'updateAdresse':{
		if(isset($_POST["ok"]) && isset($_POST['adresse']))
		{
			try
			{
				$adresse=$_POST['adresse'];
				$pdo->UpdateAdresse($adresse,$id);
				header("Location:index.php?uc=profil&action=Information");
			}
			catch(PdoExecptions $e)
			{
	...
	case 'updatePP':{
		...
		function get_extension($nom) {
			...
			$dest_dossier = 'images/'; //nom du dossier ou on stocke nos images
			$dest_fichier="";
